...
function setGraph(){
	if($("#form_hid_m_c_graph").val() != ""){
		...
		if(index_str.indexOf("1 / 2", 0) > -1){
			if(mc == "compare"){
				tracker_target_content(2);
			}else{
				tracker_target_content(3);
			}
 ...
 * 
 */
function tracker_target(mode){
	
	var reach_cost_curve = "";
	var title = "";
	title = 'Graph01: Target Reach';
	
	if(mode == 1){
		// target(reach)
		reach_cost_curve = getGraphData(1);
		
		
	}else if(mode == 2 || mode == 3){
		// target(compare)
		 var m_c_val = $("#form_hid_m_c_graph").val();
		 reach_cost_curve = m_c_val.split("|")[0];
		 //title = 'Graph01: Target Reach';
...
//		        }]
	 	...
	 	// categoryセット
	 	//options.xAxis.categories.push($("#form_hid_cat_days").val());
	 	day_list = $("#form_hid_cat_days").val().split(",");
	 	for (var i = 0; i < day_list.length; i++) {
	 		options.xAxis.categories.push(day_list[i]);
	 	}
	 
		 
		 // reach_cost_cuvreのデータを整形（40.00,60.00,20.00）
		 ...
		 for (var i = 0; i < reach_cost_curve_list.length; i++) {
			 ...
			 var adGrpNo = adGrp[0]; //広告グループID
			 var adGrpData = adGrp[1]; //広告グループのreach
			 
			 var adGrpDataList = adGrpData.split(","); //広告グループのreachのリスト
 ...
 * 
 */
function tracker_target_content(mode){
	
	var reach_cost_curve = "";
	var title = "";
	title = 'Graph02: Target Content Reach';
	
	if(mode == 1){
		// target content rate(reach)
		reach_cost_curve = getGraphData(2);
		
		
	}else if(mode == 2 || mode == 3){
		// target content rate(compare)
		 var m_c_val = $("#form_hid_m_c_graph").val();
		 reach_cost_curve = m_c_val.split("|")[1];
